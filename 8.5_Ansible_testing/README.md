...
И тест запускается. Но почему? По идее я же указал неправильный путь до докер контейнера и должна быть ошибка. <br><br> Я так понимаю, что должен запуститься docker контейнер и в нем должно пойти тестирование? У меня докер контейнер не запускается. Подскажите, в чем дело.
